    ...
    if request.method == 'POST':
        try:
            ...
            if tx_receipt and tx_receipt['status'] == 1:
                ...
                response = {'message': f'Contract created successfully', 'contract_details': contract_details}
                return JsonResponse(response)
            else:
...
#             received_json = json.loads(request.body)
#             ape_st_ff_keys = ['name', 'ein', 'locationZip', 'description', 'website', 'founder', 'jungle', 'tree', 'node']
#             if not all(key in received_json for key in ape_st_ff_keys):
#                 return HttpResponse('Some deets are missing to complete the transaction', status=400)
...
# 	}
# ]
#             response = {'message': f'Welcome to the Street, here is your ApeStFF data {index}'}
